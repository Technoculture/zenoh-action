from behaviourTree.tree import Monobehaviour, Sequence, Selector
from GetTip import GetTip

class SetTree(Monobehaviour):
    def SetupTree(self):
        discard_tip_success = Selector([GetTip().Evaluate("retry_count_below_threshold")])
        ...
        get_Tip = Sequence([GetTip().Evaluate("pickup_success"), pickup_using_orchestrator, prepare_tip_for_pickup, "tip_available"])
        return get_Tip